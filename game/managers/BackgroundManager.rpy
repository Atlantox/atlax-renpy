init python:
    class BackgroundManager:
        ...
        def TurnScreenToBlack(self, duration = None):
            targetDuration = self.blackScreenDuration if duration is None else duration

            renpy.show('blackout', layer='screens', tag='black_screen')
            renpy.transition(Dissolve(targetDuration))
            renpy.pause(targetDuration, hard=True)

        def RemoveBlackScreen(self, duration = None):
            targetDuration = self.blackScreenDuration if duration is None else duration
        ...
        def ShowMainMenuBackground(self, duration = None):
            targetDuration = self.blackScreenDuration if duration is None else duration
        ...
        def DestroyCurrentBackground(self):
            ...
            renpy.hide('bg', layer='background')   
            self.ResetBackgroundManager()